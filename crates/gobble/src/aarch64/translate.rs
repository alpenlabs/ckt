//! Aarch64-specific translation layer implementation
...
pub unsafe fn wide_hash(label: Label) -> [Label; 8] {
    let default_label = Label(unsafe { transmute::<[u8; 16], _>([0u8; 16]) });
        ...
        for test_value in [0u8, 1, 7, 15, 42, 128, 255] {
            ...
            for bit_position in 0..8 {
                ...
                let recovered_bytes: [u8; 16] =
                    unsafe { std::mem::transmute::<uint8x16_t, [u8; 16]>(recovered_labels[bit_position].0) };
                let expected_bytes: [u8; 16] =
                    unsafe { std::mem::transmute::<uint8x16_t, [u8; 16]>(expected_label.0) };
