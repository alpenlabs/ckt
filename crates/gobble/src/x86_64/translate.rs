//! x86_64-specific translation layer implementation

use crate::x86_64::{Ciphertext, Label, hash, index_to_tweak, xor128};
use std::arch::x86_64::__m128i;
use std::mem::transmute;

/// x86_64-specific bit label type, representing value 0 or 1
...
/// x86_64-specific byte label type, representing value 0-255
#[derive(Debug, Clone, Copy)]
...
/// 
/// NOTE: Translation is ONLY for INPUT encoding. Circuit outputs remain as bits.
...
pub unsafe fn wide_hash(label: Label) -> [Label; 8] {
    let default_label = Label(unsafe { transmute::<[u8; 16], __m128i>([0u8; 16]) });
    let mut labels = [default_label; 8];
    for i in 0..8 {
        labels[i] = Label(unsafe { hash(label.0, index_to_tweak(i as u64)) });
    }
    labels
}
...
/// 
/// This is for INPUT translation only. Circuit outputs remain as bits (no translation).
#[target_feature(enable = "aes")]
#[target_feature(enable = "sse2")]
pub unsafe fn generate_translation_material(
    ...
    let default_label = Label(unsafe { transmute::<[u8; 16], __m128i>([0u8; 16]) });
    let mut ciphertexts = [[Ciphertext(default_label.0); 8]; 256];
    ...
    for i in 0..256 {
        ...
        let hashed_labels = unsafe { wide_hash(input_label) };
...
pub unsafe fn translate(
    ...
    let hashed_labels = unsafe { wide_hash(byte_label_component) };
    ...
    let default_label = Label(unsafe { transmute::<[u8; 16], __m128i>([0u8; 16]) });
    let mut bit_labels = [default_label; 8];
    for bit_position in 0..8 {
...
mod tests {
    ...
    fn test_translation_roundtrip() {
        ...
        let default_label = Label(unsafe { transmute([0u8; 16]) });
        ...
        let translation_material = unsafe { generate_translation_material(byte_label, bit_labels) };
        ...
        for test_value in [0u8, 1, 7, 15, 42, 128, 255] {
            ...
            for bit_position in 0..8 {
                ...
                let recovered_bytes: [u8; 16] =
                    unsafe { transmute::<__m128i, [u8; 16]>(recovered_labels[bit_position].0) };
                let expected_bytes: [u8; 16] =
                    unsafe { transmute::<__m128i, [u8; 16]>(expected_label.0) };
