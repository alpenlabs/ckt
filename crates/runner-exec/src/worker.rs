...
pub async fn process_task<T: CircuitTask>(
    ...
    // TODO maybe do some other cleanup?

    Ok(task_output)
}

async fn process_task_inner<T: CircuitTask>(
    task_info: &T,
    ...
    let mut total_gates_processed = 0;
    while let Some(chunk) = reader.next_blocks_chunk().await? {
        ...
        for block in chunk.blocks_iter() {
            ...
            total_gates_processed += gates_in_block;
        }
