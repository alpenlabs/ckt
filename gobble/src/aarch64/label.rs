//! Aarch64-specific label types
use crate::aarch64::Label;
use crate::aarch64::{Ciphertext, hash, index_to_tweak, xor128};

/// Aarch64-specific bit label type, representing value 0 or 1
...
/// Aarch64-specific byte label type, representing value 0-255
#[derive(Debug, Clone, Copy)]
...
pub fn wide_hash(label: Label) -> [Label; 8] {
    ...
    for i in 0..8 {
        labels[i] = unsafe { std::mem::transmute(hash(label.0, index_to_tweak(i as u64))) };
    }
    labels
}
...
pub fn generate_translation_material(
    ...
    let mut ciphertexts = [[Ciphertext(Label::zero().0); 8]; 256];
    ...
    for i in 0..256 {
        ...
        // Hash the input label to get 8 labels (one for each output bit)
        let hashed_labels = wide_hash(input_label);
        ...
        for bit_position in 0..8 {
            ...
            let output_label = bit_labels[bit_position].get_label(bit_value);

            // XOR the hashed label with the output label to create the ciphertext
            let ciphertext = unsafe { xor128(hashed_labels[bit_position].0, output_label.0) };
...
pub fn translate(
    ...
    value: u8,
    translation_material: [[Ciphertext; 8]; 256],
) -> [Label; 8] {
    // Hash the byte label component to get 8 hashed labels (one for each output bit)
    let hashed_labels = wide_hash(byte_label_component);

    // Get the ciphertexts for this specific byte value
    let ciphertexts = translation_material[value as usize];

    // Decrypt each ciphertext to recover the output bit labels
    // Since C[i][j] = H_j(byte_label[i]) ⊕ bit_label[j]
    // We can recover: bit_label[j] = C[i][j] ⊕ H_j(byte_label[i])
    let mut output_labels = [Label::zero(); 8];
    for bit_position in 0..8 {
        output_labels[bit_position] =
...
mod tests {
    ...
    fn test_translation_roundtrip() {
        ...
        let mut bit_labels = [BitLabel::new([Label::zero(), Label::zero()]); 8];
        for i in 0..8 {
            bit_labels[i] = BitLabel::new([bit_labels_flat[i * 2], bit_labels_flat[i * 2 + 1]]);
        }

        // Generate translation material
        let translation_material = generate_translation_material(byte_label, bit_labels);
        ...
        for test_value in [0u8, 1, 7, 15, 42, 128, 255] {
            ...
            let recovered_labels =
                translate(byte_label_component, test_value, translation_material);
            ...
            for bit_position in 0..8 {
                ...
                let recovered_bytes: [u8; 16] =
                    unsafe { std::mem::transmute(recovered_labels[bit_position].0) };
                let expected_bytes: [u8; 16] = unsafe { std::mem::transmute(expected_label.0) };
